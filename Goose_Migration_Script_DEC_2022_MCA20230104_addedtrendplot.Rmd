...
### Import data
setwd("C:/Users/Mike/Downloads/Goose_Migration_Jan_2023-20230104T174746Z-001/Goose_Migration_Jan_2023/")

FINAL_GOOSE_DATA <- read.csv("Goose_data/FINAL_GOOSE_DATA.csv")
...
goose_jags_data <- list(
  ...
  name = as.character(FINAL_GOOSE_DATA$B_SPECIES_NAME),
  Nsubj = length(unique(FINAL_GOOSE_DATA$B_SPECIES_NAME))
)

data.frame(goose_jags_data$s, goose_jags_data$name) %>% distinct()
...
# Plot goose response
```{r}
# rename the jags model "mod" for convenience
mod <- jags_mod_7SDs

# make a dataframe of goose names
geese_names <- data.frame(num = 1:goose_jags_data$Nsubj,
                          name = sort(unique(FINAL_GOOSE_DATA$B_SPECIES_NAME)),
                          trend = round(mod$mean$beta1, 2))

# make a dataframe of years
minyr <- min(FINAL_GOOSE_DATA$BANDING_YEAR)
maxyr <- max(FINAL_GOOSE_DATA$BANDING_YEAR)
...
for (g in 1:goose_jags_data$Nsubj) {
  ...
  for (y in 1:30) {
    ...
    yslist[[y]] <- mod$sims.list$beta0[, g] +
...
# first make a dataframe of goose name and year
  # then bind it to posterior quantiles
goosesum_list[[i]] <- 
  cbind(data.frame(name = geese_names$name[i],
                 year = minyr:maxyr),
      data.frame(
do.call(rbind,
lapply(gooselist[[i]], 
...
goosesum <- do.call(rbind, goosesum_list) %>%
  rename(q2.5 = 3, q10 = 4, med = 5, q90 = 6, q97.5 = 7)

# plot the predictions

# make dataframe for trend annotations
maxdist <- goosesum %>% group_by(name) %>% summarize(max = max(q97.5))
ann <- geese_names %>% 
  left_join(maxdist) %>%
  mutate(trend2 = paste0(trend, " km/yr"))

goosesum %>%
  ggplot() +
  geom_line(aes(x = year, y = med, group = name)) +
  geom_ribbon(aes(x = year, ymin = q2.5, 
                  ymax = q97.5, group = name),
              fill = "gray", alpha = 0.6) +
...
ggsave("goose_trends.png", height = 6, width = 9, dpi = 400)
